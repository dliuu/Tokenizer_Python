...
class Tokenizer:
    ...
    def preprocess(self, text: str) -> str:
        ...
        Hints: 
            ...
            >>> 'CD ABC EF'
        """
        #text_before = re.sub(r"(\w)([(!?\"#$%&'()*+,-./\\:;=?@[\]^_`{|}~)])", r'\1 \2', text)
        text_before = re.sub(r"(\w)([(!\"#$%&'()*+,-./\\:;=?@[\]^_`{|}~])|(!\?)", r'\1 \2', text)
        text_before = re.sub(r'([,!])', r' \1', text_before)
        #clean_text = re.sub(r"(\w)([(![?\"#$%&'()*+,-./\\:;=?@[\]^_`{|}~)])", r'\1 \2', text_before)

        #clean_text = re.sub(r"(\w)([?])", r'\1 \2', clean_text)
        #clean_text = re.sub(r"(\w)([!])", r'\1 \2', clean_text)
        #print("spaces after punctuations:" + clean_text)

        clean_text = re.sub(r'\n+', ' ', text_before)
        clean_text = re.sub(r'\s+$', '', clean_text)
        clean_text = re.sub(r'\s+', ' ', clean_text)
        #print('Remove newline, whitespaces, any /n: ' + clean_text)
    ...
    def convert_tokens_to_ids(self, 
        ...
        for w in tokens:
            if w in self.word2idx:
                return_list.append(self.word2idx[w])
            else:
                return_list.append(self.word2idx.get(self.unk_token, -1))  # Return -1 if unk_token not found
        return return_list
    ...
    def encode(self, text: Union[str, List[str]], 
        ...
        for raw_text in text:
            preprocessed_text = self.word_tokenize(raw_text)

            # Convert tokens to IDs
            tokenized_text = self.convert_tokens_to_ids(preprocessed_text)

            # Padding
            if padding:
                while len(tokenized_text) < (self.maxSequenceLength - 1):
                    tokenized_text.append(self.word2idx[self.pad_token])
            
            #Truncate
            if truncate:
                tokenized_text = tokenized_text[:self.maxSequenceLength-2]

            # Add special tokens
            if add_special_tokens:
                tokenized_text.insert(0, self.word2idx[self.bos_token])
                tokenized_text.append(self.word2idx[self.eos_token])
    ...
    def create_vocab(self, fname: str, 
        ...
        if addSpecialTokens:
            ...
            self.idx2word.extend(special_tokens)
...
#tokenizer = Tokenizer(maxSequenceLength=5)
#tokenizer.load_tokenizer('cat.txt')

#print(tokenizer.preprocess("the man, who is tall, is happy!\n"))
#print(tokenizer.preprocess("The <unk> cat loves to buy food; I love him? \n\n"))

#print(tokenizer.convert_tokens_to_ids(["the", "cat"]))
#print(tokenizer.convert_tokens_to_ids(["the", "cat", "sleeps"]))
